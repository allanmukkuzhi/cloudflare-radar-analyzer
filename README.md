...
* **Fast Networking:** Implemented using Java's native `HttpClient` with non-blocking logic to ensure efficient performance.
* **Secure Login:** Uses industry-standard Bearer Token authentication for all API requests.
...
3. Replace the `apiToken` string in `radaranalyzer.java` with your key.
4. Compile and run:
   ```bash
